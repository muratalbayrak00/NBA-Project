...
input_file = "../0021500493.json"  # Girdi JSON dosyasının adı
...
for event in data["events"]:
    for moment in event["moments"]:
        ...
        game_clock = moment[2]  # Zaman bilgisi
        shot_clock = moment[3]
        period = moment[0]
        ball_holder = identify_ball_holder(moment)
        state = np.concatenate((player_positions.flatten(), ball_position, [game_clock, shot_clock, ball_holder, period])) # TODO: state'lerimize skor eklenmeli 
        if len(state) == 27:
